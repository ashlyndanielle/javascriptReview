...
function addTen(numbers) {
  ...
  for (let x = 0; x < numbers.length; x++) {
    products.push(Number(numbers[x])+10);
  }
  return products;
}
...
function longer(arr1, arr2) {
  console.log('longer', typeof(arr1.length > arr2.length ? arr1 : arr2));
...
var user2 = {
  name: 'Flippy',
  email: 'flippy@gmail.com',
  password: 'flippylovesflippy',
  username: 'flippyflop'
}
...
for (let x = 0; x < users.length; x++) {
  if (users[x].name === 'Tyler McGinnis') {
    users.splice(x, 1);
  }
}
